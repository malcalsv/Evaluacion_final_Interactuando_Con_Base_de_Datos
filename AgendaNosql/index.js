...
app.listen(8082, function () {
  console.log('Escuchando por el puerto 8082');
  CrearUsuarios("malcalsv@gmail.com","123456");
});
  ...
  UserModelo.findOne({email:user}).exec(function(err,docs){
    if(docs!=null){
      ...
      res.end("Validado");
          ...
          res.end("Usuario o contraseña no valida");
      }
    }else{
      res.end("Usuario o contraseña no valida");
    }
...
app.post("/events/delete",function(req,res){
    ...
    EventoModelo.remove({_id: id}, function(error){
      if(error){
         res.send('Error al eliminar el personaje.');
      }else{
        ...
        if(docs!=null){
          ...
          evento.save(function(error, documento){
            if(error){
               ...
               res.redirect('evento actualizado');
            }
         });
        }
    })
